...
var commitFoul = function () {
  websocket.send("addFoul", {
    Alliance: foulTeamButton.attr("data-alliance"),
    TeamId: parseInt(foulTeamButton.attr("data-team")),
    RuleId: parseInt(foulRuleButton.attr("data-rule-id")),
  });
};

// Removes the foul with the given parameters from the list.
var deleteFoul = function (alliance, team, ruleId, timeSec) {
  websocket.send("deleteFoul", {
    Alliance: alliance,
...
["R1", "R2", "R3", "B1", "B2", "B3"].forEach((allianceStation) => {
  $(`#disable-${allianceStation}`).click(() => {
    websocket.send("disableRobot", { AllianceStation: allianceStation });
  });
});
...
var cycleCard = function (cardButton) {
  ...
  websocket.send("card", {
    ...
    TeamId: parseInt($(cardButton).attr("data-card-team")),
    Card: newCard,
  });
