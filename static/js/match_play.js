...
var setAudienceDisplay = function () {
  websocket.send(
    "setAudienceDisplay",
    $("input[name=audienceDisplay]:checked").val()
  );
};
...
var setAllianceStationDisplay = function () {
  websocket.send(
    ...
    $("input[name=allianceStationDisplay]:checked").val()
  );
};
...
var handleArenaStatus = function (data) {
  ...
  $.each(data.AllianceStations, function (station, stationStatus) {
    if (stationStatus.FieldStopped) {
      $("#status" + station).addClass("field-stopped");
    } else {
      $("#status" + station).removeClass("field-stopped");
    }
    ...
    if (stationStatus.DsConn) {
      ...
      var radioOkay =
        stationStatus.Team &&
        stationStatus.Team.Id === wifiStatus.TeamId &&
        wifiStatus.RadioLinked;
      $("#status" + station + " .radio-status").attr(
        "data-status-ok",
        radioOkay
      );
      ...
      $("#status" + station + " .robot-status").attr(
        "data-status-ok",
        robotOkay
      );
      if (
        stationStatus.DsConn.SecondsSinceLastRobotLink > 1 &&
        stationStatus.DsConn.SecondsSinceLastRobotLink < 1000
      ) {
        $("#status" + station + " .robot-status").text(
          stationStatus.DsConn.SecondsSinceLastRobotLink.toFixed()
        );
      } else {
        $("#status" + station + " .robot-status").text(
          dsConn.BatteryVoltage.toFixed(1) + "V"
        );
      }
    } else {
...
var handleScoringStatus = function (data) {
  scoreIsReady =
    data.RefereeScoreReady && data.RedScoreReady && data.BlueScoreReady;
  $("#refereeScoreStatus").attr("data-ready", data.RefereeScoreReady);
  $("#redScoreStatus").text(
    "Red Scoring " +
      data.NumRedScoringPanelsReady +
      "/" +
      data.NumRedScoringPanels
  );
  $("#redScoreStatus").attr("data-ready", data.RedScoreReady);
  $("#blueScoreStatus").text(
    "Blue Scoring " +
      data.NumBlueScoringPanelsReady +
      "/" +
      data.NumBlueScoringPanels
  );
...
var handleAllianceStationDisplayMode = function (data) {
  ...
  $("input[name=allianceStationDisplay][value=" + data + "]").prop(
    "checked",
    true
  );
};
