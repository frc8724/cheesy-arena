...
var handleScorePosted = function (data) {
  ...
  var blueRankings = {};
  blueRankings[data.Match.Blue1] = getRankingText(
    data.Match.Blue1,
    data.Rankings
  );
  blueRankings[data.Match.Blue2] = getRankingText(
    data.Match.Blue2,
    data.Rankings
  );
  blueRankings[data.Match.Blue3] = getRankingText(
    data.Match.Blue3,
    data.Rankings
  );

  $("#scoreMatchName").text(
    data.MatchType + " Match " + data.Match.DisplayName
  );
  $("#redScoreDetails").html(
    matchResultTemplate({
      score: data.RedScoreSummary,
      fouls: data.RedFouls,
      rulesViolated: data.RulesViolated,
      cards: data.RedCards,
      rankings: redRankings,
    })
  );
  $("#blueScoreDetails").html(
    matchResultTemplate({
      score: data.BlueScoreSummary,
      fouls: data.BlueFouls,
      rulesViolated: data.RulesViolated,
      cards: data.BlueCards,
      rankings: blueRankings,
    })
  );
